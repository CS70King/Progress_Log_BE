...
If SMS is enabled (`NOTIFICATION_DRIVER=on`), also verify notifications:

```bash
npm run notification:test:staging -- +1YOUR_PHONE
npm run notification:test:staging -- +233XXXXXXXXX
# or for production:
npm run notification:test:production -- +1YOUR_PHONE
```

Confirm staging and production each use their own Surge and Arkesel credentials.
...
- Staging and production depend on correct external configuration:
  ...
  - Upstash Redis REST
  - SMS (when `NOTIFICATION_DRIVER=on`): separate Surge + Arkesel credentials per environment
